...
## [2025-01-20]
Добавлена надпись в левом верхнем углу карты, показывающая тип расположения судов. Если хотя бы одно судно имеет поле "вид_подхода" со значением "Фактический", отображается надпись "Фактическое расположение". Если хотя бы одно судно имеет "вид_подхода" со значением "Плановый", отображается надпись "Плановое расположение". Приоритет отдается "Фактический" над "Плановый". Добавлены стили .location-type-label с фиксированным позиционированием в левом верхнем углу и логика проверки всех судов при загрузке страницы.
...
## [2025-01-20]
Обновлена логика выбора иконок судов в зависимости от типа швартовки. Если судно пришвартовано кормой (швартовка_кормой = "Да"), используется иконка stern_ship.png, иначе используется иконка side_ship.png. Добавлена константа sternIconImagePath для пути к иконке кормы. Создана функция getIconPath для обработки путей к иконкам с поддержкой абсолютных путей Windows и file:// URL.

## [2025-01-20]
Уточнена логика применения класса lighter для иконок судов. Теперь иконка становится светлее (полупрозрачной) только если судно пришвартовано вторым бортом (швартовка_вторым_бортом = "Да"). Если судно пришвартовано кормой или обычным способом, иконка имеет обычный цвет без эффекта прозрачности.

## [2025-01-20]
Уточнено отображение судов с швартовкой вторым бортом или кормой: их иконки располагаются ближе друг к другу за счёт уменьшенных отступов, а подписи (label) разворачиваются по диагонали для визуального различия. Добавлены стили .ship-icon-wrapper.offset (новые смещения) и .ship-label.diagonal, логика назначения классов обновлена при создании иконок.
