...
function buildJsonFromObjectStrings(rawString) {
    ...
    const objectStrings = cleanedString
        .split(/}\s*[;,]?\s*{|\s*(?<=})\s*,\s*(?={)/)
        .map(chunk => {
    ...
    const jsonReadyObjects = objectStrings.map(chunk => {
        ...
        const quotedValues = quotedKeys.replace(/:\s*([^,"}{\[\]]+)(?=\s*[},])/g, (match, value) => {
            ...
            if (/^-?[\d\s]+(?:[.,]\d+)?$/.test(trimmedValue)) {
                const sanitized = trimmedValue.replace(/\s+/g, '').replace(',', '.');
                return `: ${sanitized}`;
            }
...
function normalizeNumericValue(value) {
    ...
    const numericCandidate = String(value)
        .replace(/[^-\d.,]/g, '')
        .replace(/\s+/g, '')
        .replace(',', '.');
...
function parseDataString(dataString) {
    ...
    try {
        ...
        return parsed.map(normalizeRecordValues);
    } catch (jsonError) {
        return parseObjectsStringManually(trimmedString).map(normalizeRecordValues);
    }
}
...
function parseInteractiveAreasString(rawString) {
    ...
    try {
        const parsed = JSON.parse(trimmed).map(normalizeRecordValues);
        validateInteractiveAreasData(parsed);
        return parsed;
    } catch (error) {
        const parsed = parseObjectsStringManually(trimmed).map(normalizeRecordValues);
        validateInteractiveAreasData(parsed);
        return parsed;
    }
}
...
function validateInteractiveAreasData(data) {
    ...
    data.forEach((area, index) => {
        ...
        ['x', 'y', 'width', 'height'].forEach(field => {
            if (area[field] !== undefined) {
                const numeric = normalizeNumericValue(area[field]);
                area[field] = numeric;
                if (!Number.isFinite(area[field])) {
                    throw new Error(`Поле "${field}" в области №${index + 1} должно быть числом.`);
                }
            }
        });
